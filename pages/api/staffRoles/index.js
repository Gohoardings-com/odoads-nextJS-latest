...
export const addrole = catchError(async (req, res) => {
  ...
  const result = await executeQuery(
    "INSERT into tblroles (name) VALUES ('" + name + "')",
    `odoads_${code}`
  );
  if (result) {
    all_Permission.forEach(async (el) => {
      promisies.push(
        new Promise(async (resolve, reject) => {
          const sql =
            "INSERT into tblrolepermissions SET permissionid = (SELECT permissionid from odoads_general_tbl.tblpermissions_bk WHERE shortname = '" +
            el.short_name +
            "'), can_view = " +
            el.view +
            ", can_view_own =  " +
            ...
            ",can_delete =  " +
            el.delete +
            ", roleid =(SELECT Max(roleid) from  tblroles WHERE name = '" +
            name +
            "') ";
          const result = await executeQuery(sql, `odoads_${code}`);
          if (result) {
            resolve(result);
          } else {
            reject(result);
...
export const allrole = catchError(async (req, res) => {
  ...
  const data = await executeQuery(
    "SELECT name, roleid From tblroles where status=1",
    `odoads_${code}`
  );
  if (data) {
...
export const deleterole = catchError(async (req, res) => {
  ...
  if (code) {
    const { name } = req.body;
    const data = await executeQuery(
      "DELETE FROM tblroles WHERE name = '" + name + "'",
      `odoads_${code}`
    );
    if (data) {
...
export const updaterole = catchError(async (req, res) => {
  ...
  const data = await executeQuery(
    "SELECT can_view  FROM tblrolepermissions  WHERE roleid = " +
      id +
      " LIMIT 1",
    `odoads_${code}`
  );
      ...
      if (data.length == 0) {
        ...
        roleData.forEach(async (el) => {
          const value = await executeQuery(
            "UPDATE tblrolepermissions SET  can_view = " +
              el.can_view +
              ", can_view_own =  " +
              el.can_view_own +
              ",  can_edit =  " +
