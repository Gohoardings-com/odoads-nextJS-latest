...
export const addStaff = catchError(async (req, res) => {
  ...
  const addStaff = await executeQuery(
    "INSERT into tblstaff SET email = '" +
      ...
      "', password = '" +
      newPassword +
      "', datecreated = CURRENT_TIMESTAMP, active = 1,  role = " +
      role_name +
      "",
    `odoads_${code}`
  );
    ...
    all_Permission.map(async (el) => {
      promises.push(
        new Promise(async (resolve, reject) => {
          const addPermissions = await executeQuery(
            "INSERT into tblstaffpermissions SET staffid  = (SELECT Max(staffid) from tblstaff WHERE  role = " +
              role_name +
              "), can_view = " +
              el.view +
              ", can_view_own = " +
              ...
              ", can_delete = " +
              el.delete +
              ", permissionid = (SELECT permissionid from odoads_general_tbl.tblpermissions_bk WHERE shortname = '" +
              el.short_name +
              "')",
            `odoads_${code}`
          );
  ...
  try {
    ...
    if (data) {
      return res
        .status(200)
        .json({ message: "Satff Member added Successfully" });
    }
  } catch (err) {
    return res.send(err);
  }
...
export const updateSatff = catchError(async (req, res) => {
  ...
  const promises = [];
  const { email, staffid, role, firstname, password, facebook, linkdin, skype, lastname, phonenumber } = value;

  const newPassword = password && password.length >= 8 ? bcrypt.hashSync(String(password), 8) : undefined;
  ...
  const commonUpdateFields = [
    ...
    `role = (SELECT Max(roleid) FROM tblroles WHERE name LIKE '%${role}%')`,
  ];

  const passwordUpdateFields = newPassword ? [`password = '${newPassword}'`] : [];

  const updateFields = [...commonUpdateFields, ...passwordUpdateFields];

  const updateStaffQuery = `UPDATE tblstaff SET ${updateFields.join(',')} WHERE staffid = ${staffid}`;

  const addStaff = await executeQuery(updateStaffQuery, `odoads_${code}`);
    ...
    new Promise(async (resolve, reject) => {
      if (data.length === 0) {
        permission.map(async (el) => {
          ...
          addPermissions ? resolve(addPermissions) : reject(addPermissions);
        });
      } else {
        permission.map(async (el) => {
          const updatePermissions = await executeQuery(`UPDATE tblstaffpermissions SET can_view = ${el.can_view}, can_view_own = ${el.can_view_own}, can_edit = ${el.can_edit}, can_create = ${el.can_create}, can_delete = ${el.can_delete} WHERE staffid = ${staffid} AND permissionid = (SELECT permissionid FROM odoads_general_tbl.tblpermissions_bk WHERE shortname = '${el.short_name}')`, `odoads_${code}`);
          updatePermissions ? resolve(updatePermissions) : reject(updatePermissions);
        });
      }
    })
  );
              ...
              if (filename) {
                ...
                if (staffid) {
                  ...
                  if (name == "media") {
                    ...
                    query =
                      " WHERE staffid = (SELECT Max(staffid) FROM tblstaff)";
                  }
                }
                ...
                switch (name) {
                  case "media":
                    data = await executeQuery(
                      "Update tblmedia_deails SET thumbnail = '" +
                        image +
                        "' " +
                        query +
                        "",
                  ...
                  case "staff":
                    data = await executeQuery(
                      "Update tblstaff SET profile_image = '" +
                        image +
                        "' " +
                        query +
                        "",
